...
const transporter = nodemailer.createTransport({
  ...
  auth: {
    ...
    pass: (process.env.EMAIL_KEY || process.env.EMAIL_PASS || "").replace(/\s+/g, "").trim(),
  },
});

/**
 * Send email using Nodemailer with HTML and CSS support
 * Accepts both positional arguments (email, subject, text, html) or an options object ({ email/to, subject, text, html })
 */
const sendEmail = async (emailOrOptions, subject, text, html) => {
  try {
    let email, mailSubject, mailText, mailHtml;

    if (typeof emailOrOptions === "object" && emailOrOptions !== null) {
      email = emailOrOptions.email || emailOrOptions.to;
      mailSubject = emailOrOptions.subject;
      mailText = emailOrOptions.text;
      mailHtml = emailOrOptions.html;
    } else {
    ...
    const mailOptions = {
      from: `"PassOP Security" <${fromAddress}>`,
      to: email,
      subject: mailSubject,
      text: mailText,
...
/**
 * Modern HTML & CSS template for 6-digit OTP verification email
 */
...
/**
 * Modern HTML & CSS template for direct link password reset email
 */
